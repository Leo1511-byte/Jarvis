...
Milestone 32's Rust half (below) — needs `cargo`, never compiled, see
[`docs/archive/HANDOFF_M32_MULTIWINDOW_RUST_VERIFICATION.md`](docs/archive/HANDOFF_M32_MULTIWINDOW_RUST_VERIFICATION.md).
...
| 32 | Multi-window foundation | **Frontend done + live-verified, 2026-08-11; Rust half unverified (needs `cargo`).** Any real view (Dashboard/Chat/Projects/Tasks/Memory/Agents/Integrations/Activity) can pop out into its own Tauri window via a Sidebar button — `?view=<slug>` loads the same bundle standalone, no shared cross-window state sync (each window independently reads the same store). Voice listener deliberately disabled outside the main window (one global backend process, not per-window). See handoff doc above for what still needs a real `cargo build`/click-through |
